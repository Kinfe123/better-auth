...
`schema` <span className="opacity-70">`InferOptionSchema>`</span>
...
`permissions` <span className="opacity-70">`{ defaultPermissions?: Statements | ((referenceId: string, ctx: GenericEndpointContext) => Statements | Promise) }`</span>
...
Read more about permissions [below](#permissions).
    ...
    `defaultPermissions` <span className="opacity-70">`Statements | ((referenceId: string, ctx: GenericEndpointContext) => Statements | Promise)`</span>
...
## Generated Context
This page is grounded in the repository's existing documentation at `docs/content/docs/plugins/api-key/reference.mdx`, then placed into the generated Docs Cloud sidebar for Better Auth.
## Maintainer Notes
