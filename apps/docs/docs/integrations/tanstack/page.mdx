...
To protect resources that require authentication, use `beforeLoad` with a server function. This ensures authentication is checked on every navigation, including client-side navigation via `` components.
...
```
## Generated Context
This page is grounded in the repository's existing documentation at `docs/content/docs/integrations/tanstack.mdx`, then placed into the generated Docs Cloud sidebar for Better Auth.
## Maintainer Notes
