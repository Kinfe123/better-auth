...
RSCs that run after the response has started streaming cannot set cookies. The [cookie cache](/docs/concepts/session-management#cookie-cache) will not be refreshed until the server is interacted with from the client via Server Actions or Route Handlers.
...
```
## Generated Context
This page is grounded in the repository's existing documentation at `docs/content/docs/integrations/waku.mdx`, then placed into the generated Docs Cloud sidebar for Better Auth.
## Maintainer Notes
